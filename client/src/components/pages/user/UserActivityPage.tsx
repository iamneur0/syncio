...
export default function UserActivityPage() {
  ...
  const [viewMode, setViewMode] = useState<'card' | 'list'>(() => {
    if (typeof window !== 'undefined') {
      const raw = String(localStorage.getItem('user-activity-view-mode') || 'card').toLowerCase().trim()
  ...
  const { data: libraryData, isLoading: isLoadingLibrary, isFetching: isFetchingLibrary, refetch: refetchLibrary } = useQuery({
    queryKey: ['user-activity-library', userId, userFilter, publicUserIds],
    queryFn: async () => {
  ...
  // Filter for activity/history (watched items only)
  const { filteredItems, groupedByDate } = useMemo(() => {
    ...
    // Activity mode: show ALL watched items
    ...
    <>
      ...
      {isLoadingLibrary && !libraryData ? (
        ...
        <EmptyState
          ...
          description={searchTerm ? 'Try adjusting your search terms' : 'No watched items to display'}
        />
      ) : (
